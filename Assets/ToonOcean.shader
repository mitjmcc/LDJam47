Shader "Custom/ToonOcean"
{
    Properties {
        ...
        _Spark_Em_Color("Spark_Em_Color", Color) = (0,0.8904905,1,0)
    ...
    SubShader {
        ...
        uniform sampler2D _SparkleNoiseTexture;
        uniform float2 _Spark_Noise_Speed;
        uniform float4 _SparkleNoiseTexture_ST;
        uniform float _Spark_Em_Mult;
        uniform float4 _Spark_Em_Color;
        ...
        void surf (Input i, inout SurfaceOutput1 o) {
            ...
            // Sparkle
            float2 uv_SparkleTexture = i.uv_MainTex * _SparkleTexture_ST.xy + _SparkleTexture_ST.zw;
            float4 tex2DNode1 = tex2D( _SparkleTexture, uv_SparkleTexture );
            float2 uv_SparkleNoiseTexture = i.uv_MainTex * _SparkleNoiseTexture_ST.xy + _SparkleNoiseTexture_ST.zw;
            float2 panner3 = ( 1.0 * _Time.y * _Spark_Noise_Speed + uv_SparkleNoiseTexture);
            float4 temp_output_7_0 = ( ( ( tex2DNode1 * tex2D( _SparkleNoiseTexture, panner3 ) ) * _Spark_Em_Mult ) * _Spark_Em_Color );

            col *= _Color;
            o.Albedo = rampCol.rgb + temp_output_7_0.rgb;
